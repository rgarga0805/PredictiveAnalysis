    ...
    var myController = function ($scope, $sce, $http, $compile) {
        ...
        var responsePromise = $http.get(CONFIG.get('GET_CUSTOMER_DETAILS')).then(function (response, status, headers, config) {
            ...
            $http.get(CONFIG.get('GET_KPI_LIST')).then(function (response, status, headers, config) {
               ...
               // $scope.KPIPerCustomer = response.data;
               // console.log($scope.KPIPerCustomer);
               // KPIInfo(response.data);
               // console.log($scope.KPIPerCustomer);
                console.log("customerInfo:" + $scope.ChurnModelInfo);
               // KPIInfo(response.data);
            },
        ...
        //function KPIInfo(data)
        //{
        //    $scope.KPIPerCustomer = [];
        //    angular.forEach(data, function (value, key) {
        //        var data = new Object();
        //        data.Id = value.Id;
        //        data.DisplayName = value.DisplayName;
        //        data.checked = false; 
        //        $scope.KPIPerCustomer.push(data);
        //    });
        //   // $scope.KPIPerCustomer = { data, selected:false};
        ...
        function createArrayDataTable(dataset)
        {
            ...
            $('.myDataTable').html('<thead><tr><th>#</th><th>Name</th><th>On Time Delivery</th><th>Total Net Revenue</th><th>Age</th></tr></thead>');
           // $compile(test)($scope)
        ...
        $scope.callNPLResult = function()
        {
            var responsePromise = $http.get(CONFIG.get('GET_NPL_URL')).then(function (response, status, headers, config) {
                ...
                $scope.customerFeedbackReviewBool = response.data.sentiment.document.score.label;
            },
                function (data, status, headers, config) {
                    alert(CONFIG.get('API_FAIL_TEXT'));
                });
